...
class CampaignCleanupService
{
     ...
     */
    public function cleanup(CampaignDefinition $definition): array
    {
        $summary = [
            'ready' => 0,
            'needs_email' => 0,
        ...
        $this->prospectsQuery($definition)->orderBy('id')->chunkById(100, function ($prospects) use (&$summary, $definition): void {
            foreach ($prospects as $prospect) {
                $changes = $this->normalizedState($prospect, $definition);
                $prospect->forceFill($changes)->save();
                $summary['updated']++;
            }
        });

        $summary['ready'] = $this->prospectsQuery($definition)->where('lifecycle_status', GrowthProspect::LIFECYCLE_READY)->count();
        $summary['needs_email'] = $this->prospectsQuery($definition)->where('email_status', GrowthProspect::EMAIL_STATUS_MISSING)->count();
        $summary['invalid_email'] = $this->prospectsQuery($definition)->where('email_status', GrowthProspect::EMAIL_STATUS_INVALID)->count();
        $summary['manual_review'] = $this->prospectsQuery($definition)
            ->where('lifecycle_status', GrowthProspect::LIFECYCLE_MANUAL_REVIEW)
            ->count();
        $summary['duplicates'] = $this->prospectsQuery($definition)->whereNotNull('duplicate_of_id')->count();
     ...
     */
    public function attentionRecords(CampaignDefinition $definition, int $limit = 20): Collection
    {
        return $this->prospectsQuery($definition)
     ...
     */
    private function normalizedState(GrowthProspect $prospect, CampaignDefinition $definition): array
    {
        ...
        $qualityManualReview = $this->qualityManualReview($prospect, $definition);
    ...
    private function qualityManualReview(GrowthProspect $prospect, CampaignDefinition $definition): bool
    {
    ...
    private function prospectsQuery(CampaignDefinition $definition): Builder
    {
        $campaignId = $this->campaign($definition)->id;

        return GrowthProspect::query()
            ->where(function (Builder $query) use ($campaignId, $definition): void {
                $query->where('campaign_id', $campaignId)
                    ->orWhere('last_campaign_slug', $definition->slug());
            });
    }

    private function campaign(CampaignDefinition $definition): GrowthCampaign
    {
