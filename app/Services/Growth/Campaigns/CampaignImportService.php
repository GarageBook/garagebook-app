...
class CampaignImportService
{
     ...
     */
    public function importPath(CampaignDefinition $definition, string $path): array
    {
        $rows = $this->readRows($path);
        $campaign = $this->campaign($definition);
        ...
        foreach ($rows as $row) {
            ...
            $this->events->log($prospect, GrowthOutreachEvent::TYPE_IMPORTED, $campaign, $definition->slug(), null, ['source' => $row]);
            if ($eventType === GrowthOutreachEvent::TYPE_ENRICHED) {
                $this->events->log($prospect, GrowthOutreachEvent::TYPE_ENRICHED, $campaign, $definition->slug(), null, ['source' => $row]);
                $result['enriched']++;
            }
            $result['imported']++;
        }
    ...
    public function campaign(CampaignDefinition $definition): GrowthCampaign
    {
