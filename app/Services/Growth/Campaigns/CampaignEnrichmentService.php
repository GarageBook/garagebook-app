...
class CampaignEnrichmentService
{
     ...
     */
    public function enrich(CampaignDefinition $definition, ?int $limit = null): array
    {
        $summary = [
            'scanned' => 0,
            'auto_found' => 0,
        ...
        $query = $this->eligibleProspectsQuery($definition)->orderBy('id');

        if ($limit !== null && $limit > 0) {
            $query->limit($limit);
        }

        $query->get()->each(function (GrowthProspect $prospect) use (&$summary, $definition): void {
            $summary['scanned']++;
            $candidate = $this->findBestEmail($prospect, $definition);

            if ($candidate === null) {
                $prospect->forceFill([
                    'enrichment_notes' => $this->appendNote($prospect, $definition->slug().'_enrichment: geen bruikbaar publiek e-mailadres gevonden'),
                ])->save();
        ...
        $summary['still_missing'] = $this->missingWithoutSuggestionQuery($definition)->count();
        $summary['ready_top_50'] = $this->readyTop50($definition);

        return $summary;
    }

    private function eligibleProspectsQuery(CampaignDefinition $definition): Builder
    {
        ...
        return GrowthProspect::query()
            ...
            ->where(function (Builder $query) use ($campaign, $definition): void {
                $query->where('campaign_id', $campaign->id)
                    ->orWhere('last_campaign_slug', $definition->slug());
            });
    }

    private function missingWithoutSuggestionQuery(CampaignDefinition $definition): Builder
    {
        $campaign = $this->importer->campaign($definition);
     ...
     */
    private function findBestEmail(GrowthProspect $prospect, CampaignDefinition $definition): ?array
    {
        ...
        if ($home !== null) {
            ...
            $best = $this->bestCandidate($candidates, $definition);
        ...
        return $this->bestCandidate($candidates, $definition);
    }
     ...
     */
    private function bestCandidate(array $candidates, CampaignDefinition $definition): ?array
    {
     ...
     */
    private function readyTop50(CampaignDefinition $definition): array
    {
        $campaign = $this->importer->campaign($definition);
