...
class CampaignQualityFilter
{
    ...
    public function assess(DiscoveryResult $result, CampaignDefinition $definition): DiscoveryResult
    {
        $score = 100;
        $flags = [];
        ...
        if ($subtype !== null && $allowedSubtypes !== [] && ! in_array($subtype, $allowedSubtypes, true)) {
            return $result->withQuality(0, ['invalid_subtype'], 'rejected', 'invalid prospect subtype');
        }

        if ($hasWebsite && Str::contains((string) ($result->notes ?? ''), $definition->seedLabel().' seed URL')) {
            return $result->withQuality(60, ['seed_url_fallback', 'no_email', 'manual_review_required'], 'manual_review', $definition->seedLabel().' seed URL fallback needs enrichment');
        }
    ...
    private function reasonForVerdict(string $verdict, array $flags, bool $hasWebsite, bool $hasEmail, ?string $subtype): ?string
    {
        ...
        if (in_array('weak_website_signal', $flags, true)) {
            return 'weak website signal';
        }

        if (in_array('seed_url_fallback', $flags, true)) {
            return 'seed url fallback';
        }
