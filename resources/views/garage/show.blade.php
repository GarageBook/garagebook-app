...
@section('content')
    <article style="max-width:1120px; margin:0 auto; padding:40px 24px 88px;">
        ...
        <header style="display:grid; gap:24px; margin-bottom:32px;">
            <div style="display:grid; gap:18px; padding:32px; border-radius:32px; background:linear-gradient(140deg, #fff9dd 0%, #ffffff 46%, #eef4ff 100%); border:1px solid #e5e7eb; box-shadow:0 20px 60px rgba(15, 23, 42, 0.06);">
                <div style="display:flex; flex-wrap:wrap; gap:12px; align-items:center; justify-content:space-between;">
                    <span style="display:inline-flex; width:fit-content; padding:8px 12px; border-radius:999px; background:#fff1a8; color:#111827; font-weight:700;">
                        Gedeeld door eigenaar
                    </span>
                    <span style="display:inline-flex; width:fit-content; padding:8px 12px; border-radius:999px; background:#eef2ff; color:#3730a3; font-weight:600;">
                        Eigenaar houdt controle over wat openbaar is
                    </span>
                </div>

                <div style="display:grid; gap:14px; max-width:820px;">
                    <h1 style="margin:0; font-size:clamp(2.2rem, 4.4vw, 3.6rem); line-height:1.02; letter-spacing:-0.03em; color:#111827;">
                ...
                <div style="display:flex; flex-wrap:wrap; gap:12px;">
                    ...
                    <a
                        ...
                        style="display:inline-flex; align-items:center; justify-content:center; padding:12px 18px; border-radius:999px; background:#fff; color:#111827; text-decoration:none; font-weight:700; border:1px solid #d1d5db;"
                    >
        ...
        <section style="display:grid; gap:16px; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); margin-bottom:24px;">
        ...
        <section style="display:grid; gap:16px; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); margin-bottom:40px;">
        ...
        <section style="display:grid; grid-template-columns:minmax(0, 1.6fr) minmax(280px, 0.9fr); gap:20px; margin-bottom:40px;">
            <article style="padding:24px; border-radius:28px; background:#fff; border:1px solid #e5e7eb;">
                <h2 style="margin:0 0 12px; font-size:28px; color:#111827;">Waarom deze historie vertrouwen opbouwt</h2>
            ...
            <aside style="padding:24px; border-radius:28px; background:linear-gradient(135deg, #111827 0%, #1f2937 100%); color:#fff;">
                ...
                <h2 style="margin:0 0 10px; font-size:26px; line-height:1.08;">{{ $shareCues['title'] }}</h2>
        ...
        <section style="margin-bottom:40px;">
            <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:16px; flex-wrap:wrap;">
                <div>
                    <h2 style="margin:0; font-size:28px; color:#111827;">Onderhoudstijdlijn</h2>
            ...
            <div style="display:grid; gap:20px;">
                @forelse($timelineItems as $item)
                    <article style="padding:22px; border-radius:24px; border:1px solid #e5e7eb; background:#fff; box-shadow:0 16px 36px rgba(15, 23, 42, 0.04);">
                        <div style="display:grid; gap:12px;">
                            ...
                            <h3 style="margin:0; font-size:21px; line-height:1.25; color:#111827;">{{ $item['description'] !== '' ? $item['description'] : 'Onderhoudsmoment' }}</h3>

                            @if($item['evidence_labels'] !== [])
                                <div style="display:flex; flex-wrap:wrap; gap:8px;">
                                    @foreach($item['evidence_labels'] as $label)
                                        <span style="display:inline-flex; padding:7px 10px; border-radius:999px; background:#eef6ff; color:#1d4ed8; font-size:13px; font-weight:600;">
                                            {{ $label }}
                                        </span>
                                    @endforeach
                                </div>
                            ...
                            @if($item['notes'])
                                <p style="margin:0; color:#374151; white-space:pre-line; line-height:1.7;">{{ $item['notes'] }}</p>
                            @endif
        ...
        <section style="padding:24px; border-radius:28px; background:linear-gradient(135deg, #111827 0%, #1f2937 100%); color:#fff; margin-bottom:32px;">
            <h2 style="margin:0 0 12px; font-size:28px;">Bouw aan een overdraagbare voertuiggeschiedenis</h2>
        ...
        <section style="display:grid; gap:12px;">
            <h2 style="margin:0; font-size:24px; color:#111827;">Ook je onderhoud digitaal bijhouden?</h2>
