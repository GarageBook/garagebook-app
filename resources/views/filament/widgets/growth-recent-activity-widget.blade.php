                ...
                @foreach ($sections as $section)
                    <article class="rounded-[1.75rem] border border-slate-200/80 bg-gradient-to-b from-white to-slate-50/70 p-5 shadow-sm">
                        <div class="mb-4 flex items-center justify-between gap-3">
                            ...
                            <span class="inline-flex items-center rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-slate-500">Laatste 5</span>
                        </div>
                                    ...
                                    <div class="flex items-start justify-between gap-3 rounded-[1.25rem] border border-slate-200/80 bg-white px-4 py-3 shadow-sm">
                                        ...
                                        <div class="shrink-0 text-right">
                                            @if (! empty($row['source']) && $row['source'] !== '—')
                                                <span class="inline-flex items-center rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-semibold text-sky-700 ring-1 ring-sky-200">{{ $row['source'] }}</span>
                                            @else
